...
輸入報表資料夾與桿號（poles_id），自動比對每日 Excel/CSV 報表中的 controller_id，
產出該桿號的歷史異動軌跡。**純前端執行，資料不會上傳到任何伺服器**——所有解析與比對
都在你的瀏覽器裡完成，只要選好資料夾就能分析，不需要安裝任何軟體、不需要架設後端。
...
全為虛構的 xlsx/csv 檔案（不含任何真實座標、IMEI、地址），方便本機測試分析流程：
...
- **日期判定**：CSV 內 `date:YYYY-MM-DD` 標籤 → 檔名/父資料夾路徑（如 `202606/0609...`）
  → 檔案最後修改時間，三層 fallback。
- **同日多檔案取捨**：以檔名內時間戳記或檔案最後修改時間較晚者為準。
- **xlsx 多 Sheet**：依序查找 `Receive<10` → `Receive≠0` → 其他 Sheet，取第一筆符合的紀錄。
